...
from schemas.sale_item import SaleItemUpdate, SaleItemCreate, SaleItemRead
...
@router.get("/{id}", response_model=SaleItemRead)
def get_sale_item(id: int, db: Session = Depends(get_db)):
    return sale_item_services.get_sale_item(db, id)
...
def create_sale_item(data: SaleItemCreate, db: Session = Depends(get_db)):
    return sale_item_services.create_sale_item(db, data)

@router.put("/{sale_item_id}", response_model=SaleItemRead)
def update_sale_item(
    sale_item_id: int, data: SaleItemUpdate, db: Session = Depends(get_db)
):
    return sale_item_services.update_sale_item(db, sale_item_id, data)

@router.delete("/{sale_item_id}", response_model=SaleItemRead)
def delete_sale_item(sale_item_id: int, db: Session = Depends(get_db)):
    return sale_item_services.delete_sale_item(db, sale_item_id)
